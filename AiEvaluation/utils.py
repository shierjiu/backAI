...
from AiEvaluationy.models import AiEvaluationEntity
...
def build_entity_config(entity: AiEvaluationEntity, timeout=None) -> dict:
    try:    headers = json.loads(entity.header) if entity.header else {}
    except: headers = {}
    try:    body    = json.loads(entity.body)   if entity.body   else {}
    except: body    = {}
    return {"name": entity.name.lower(),"url": entity.url,"method": (entity.method or "POST").upper(),"headers": headers,"body": body,"stream": entity.stream,"timeout":  timeout,"response_function": entity.response_function or "",}
