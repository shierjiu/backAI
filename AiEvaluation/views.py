...
from .models import *
from .utils import build_entity_config
...
# 评估器
class AiEvaluationRecordServer(APIView):
    def post(self, request):
        dataset = request.data.get('dataset')
        agent = request.data.get('agent')
        # 1. 必填校验
        if not dataset or not agent:
            return api_response(status=400, message="缺少 dataset 或 agent")
        if not isinstance(agent, list):
            return api_response(status=400, message="agent 必须是列表")

        # 2. 验证数据集
        try:
            dataset_obj = AiEvaluationDataset.objects.get(pk=dataset)
        except AiEvaluationDataset.DoesNotExist:
            return api_response(status=404, message="数据集不存在")

        # 3. 验证智能体
        agents_qs = AiServerAgent.objects.filter(id__in=agent)
        if agents_qs.count() != len(agent):
            return api_response(status=404, message="所选智能体中有一个或多个不存在")

        # 4. 验证条目一致性 & 获取 entity
        entries = AiEvaluationDatasetEntry.objects.filter(dataset=dataset_obj)
        if not entries.exists():
            return api_response(status=400, message="数据集没有可评测的条目")
        distinct_entity_ids = entries.values_list('entity', flat=True).distinct()
        if distinct_entity_ids.count() > 1:
            return api_response(status=400, message="数据集条目评测对象不一致，请统一后再评测")
        target_entity_id = distinct_entity_ids.first()
        if target_entity_id is None:
            return api_response(status=400, message="条目中没有绑定测评对象")
        try:
            entity_obj = AiEvaluationEntity.objects.get(pk=target_entity_id)
        except AiEvaluationEntity.DoesNotExist:
            return api_response(status=404, message="测评对象不存在")

        # 5. 创建历史记录（status 默认为 'running'）
        ts = timezone.now().strftime('%Y%m%d%H%M')
        dataset_name = dataset_obj.name
        agent_names = [AiServerAgent.objects.get(pk=aid).name for aid in agent]
        entity_name = entity_obj.name

        record = AiEvaluationRecord.objects.create(
            name=f"{entity_name}-{dataset_name}-{ts}",
            dataset=dataset_name,
            agent=",".join(agent_names),
            entity=entity_name,
            status='running',  # 显式标记为运行中
        )

        serializer = AiEvaluationRecordSerializer(record)

        # 6. 启动后台线程
        worker = threading.Thread(
            target=self._run_evaluate_by_agents,
            args=(record.id, dataset_obj, agent),
            daemon=True
        )
        worker.start()
        ...
        return api_response(
            status=200,
            message="评估已开始，可在历史记录中查看结果",
            data={"record": serializer.data}
        )

    def _run_evaluate_by_agents(self, record_id, dataset_obj, agent_ids):
        try:
            ...
            for entry in entries:
                single = {"question": entry.question,"ground_truth": entry.ground_truth or "","contexts": entry.contexts or "","answer": entry.answer or ""}
                for aid in agent_ids:
                    agent_obj = AiServerAgent.objects.get(pk=aid)
                    cfg = AiServerAgentSerializer(agent_obj).data
                    parts = [f"Question: {entry.question}",f"Ground_truth: {entry.ground_truth or ''}",*([f"Contexts: {entry.contexts}"] if entry.contexts else []),f"Answer: {entry.answer or ''}",]
                    user_content = "\n".join(parts)

                    try:
                        raw = AIAgentServer(agent_config=cfg, user_content=user_content)\
                                  .agent_server().strip()
                ...
                AiEvaluationRecordEntry.objects.create(
                    record=record,
                    result=json.dumps(single, ensure_ascii=False)
                )

            # 全部完成后，更新状态
            record.status = 'completed'
            record.save(update_fields=['status'])
            logger.info(f"异步评测完成 record_id={record.id}")
    ...
    def post(self, request):
        ...
        if dataset_id is not None:
            ...
            try:
                ...
                return api_response(status=400, message="数据集ID无效")
            # 在 pageRule 里追加过滤 dataset 字段的规则
            request.data.setdefault('pageRule', [])
            request.data['pageRule'].append({'field': 'dataset', 'rule': 'is', 'value': dataset_id, })

        # 2. 调用公共分页查询（如果没传 dataset，就相当于不过滤 dataset）
        return pagination_query(cls=AiEvaluationDatasetEntry, cls_serializer=AiEvaluationDatasetEntrySerializer,
                                request=request)
...
# 历史测评对象记录
class AiEvaluationHistoryList(APIView):

    parser_classes = (JSONParser,)
    def post(self, request):
        """
        POST /ai_evaluation/history/list
        直接根据 record 全量查询对应的历史明细列表
        """
        # 传record 字段
        record_id = request.data.get('record')
        if not record_id:
            return api_response(status=400, message="缺少记录ID")
        try:
            record_id = int(record_id)
        except (TypeError, ValueError):
            return api_response(status=400, message="记录ID无效")
        # 全量查询
        qs = AiEvaluationRecordEntry.objects.filter(record_id=record_id)
        serializer = AiEvaluationRecordEntrySerializer(qs, many=True)
        return api_response(status=200, message="查询成功", data=serializer.data)


class AiEvaluationHistoryInfo(APIView):
    def get(self, request):
...
class ExcelImportView(APIView):
    ...
    def post(self, request):
        ...
        if not dataset_id or not json_data:
            return api_response(status=400,message="缺少 dataset 或 json_data 参数",data={"created": 0, "skipped": 0})
        # 2. 验证数据集是否存在
        try:
            dataset_obj = AiEvaluationDataset.objects.get(pk=int(dataset_id))
        except (ValueError, AiEvaluationDataset.DoesNotExist):
            return api_response(status=404,message=f"数据集 ID={dataset_id} 不存在",data={"created": 0, "skipped": 0})
        # 3. 解析 json_data
        try:
            ...
            return api_response(status=400,message="json_data 不是合法的 JSON 数组",data={"created": 0, "skipped": 0})
        # 4. 缓存 tag 和 entity（按名称匹配）
        tag_cache    = {t.name: t for t in AiEvaluationDatasetTag.objects.all()}
        entity_cache = {e.name: e for e in AiEvaluationEntity.objects.all()}
        created_count = 0
        skipped_count = 0
        ...
        for rec in records:
            ...
            # 解析更新时间
            parsed_dt = None
            ut = rec.get('update_time')
            if ut:
                for fmt in ("%Y-%m-%d %H:%M:%S", "%Y/%m/%d %H:%M"):
                    try:
                        parsed_dt = datetime.datetime.strptime(ut, fmt)
                        break
                    except ValueError:
                        pass
            tag_obj    = tag_cache.get(rec.get('tag'))
            entity_obj = entity_cache.get(rec.get('entity'))

            AiEvaluationDatasetEntry.objects.create(question = q.strip(),ground_truth = g,answer = answer,contexts = contexts, update_time  = parsed_dt, tag = tag_obj,entity = entity_obj,dataset = dataset_obj)
            created_count += 1

        # 6. 返回结果，data 永远有值
        return api_response(status=200,message=f"导入完成：{created_count} 条新建，{skipped_count} 条跳过",data={"created": created_count, "skipped": skipped_count})


# 一键更新
class DatasetEvaluateView(APIView):
    """
    POST /ai_evaluation/entity/server
    { entity:1 }   调试
    { entity:1, dataset:1 }  批量更新
     {
      "entity": 1,
      "detail": 1    对单条明细做更新
    }
    调试模式：仅传 entity；批量模式：传 dataset 和 entity 并异步更新明细。 单条数据：传 entity 和 detail
    """

    def post(self, request):
        dataset = request.data.get('dataset') or request.query_params.get('dataset')
        entity  = request.data.get('entity')  or request.query_params.get('entity')
        detail  = request.data.get('detail')  or request.query_params.get('detail')
        # 转换为 int 或 None
        try:    dataset = int(dataset) if dataset is not None else None
        except: dataset = None
        try:    entity  = int(entity)  if entity  is not None else None
        except: entity  = None
        try:    detail  = int(detail)  if detail  is not None else None
        except: detail  = None

        # ----- 单条明细评测 -----
        if entity and detail:
            # 验证测评对象
            try:
                entity_obj = AiEvaluationEntity.objects.get(pk=entity)
            except AiEvaluationEntity.DoesNotExist:
                return api_response(status=400, message='测评对象 ID 不存在')

            # 验证明细条目
            try:
                entry = AiEvaluationDatasetEntry.objects.get(pk=detail)
            except AiEvaluationDatasetEntry.DoesNotExist:
                return api_response(status=404, message=f"数据明细 ID={detail} 不存在")

            # 重置并绑定
            entry.answer      = ''
            entry.update_time = None
            entry.entity      = entity_obj
            entry.save(update_fields=['answer', 'update_time', 'entity'])
            # 调用 AI
            cfg = build_entity_config(entity_obj)
            cfg['body'] = self._build_body_with_exec(cfg.get('body', {}), entry.question)
            ...
            if resp.get('answer'):
                ...
                return api_response(status=200,message='单条数据评测完成',data={'detail_id': detail, 'answer': resp['answer']})
            else:
                entry.status = 'false'
                entry.save(update_fields=['status'])
                return api_response(status=500,message=resp.get('error', '第三方接口调用错误'))
        # ----- 调试模式：仅 entity（Hello world） -----
        if entity and not dataset:
            try:
                entity_obj = AiEvaluationEntity.objects.get(pk=entity)
            except AiEvaluationEntity.DoesNotExist:
                return api_response(status=400, message='测评对象 ID 不存在')
            cfg = build_entity_config(entity_obj)
            cfg['body'] = self._build_body_with_exec(cfg.get('body', {}), "Hello, world!")
            resp = GenericAIService.get_answer(cfg)
            if resp.get('answer'):
                return api_response(status=200, message='调试评测完成', data={'answer': resp['answer']})
            return api_response(status=500, message=resp.get('error', '未知错误'))

        # ----- 批量评测：dataset + entity -----
        if dataset and entity:
            # 先在同步阶段验证 dataset 和 entity，保证前端能拿到错误
            try:
                dataset_obj = AiEvaluationDataset.objects.get(pk=dataset)
            except AiEvaluationDataset.DoesNotExist:
                return api_response(status=404, message=f"数据集 ID={dataset} 不存在")
            ...
            worker = threading.Thread(
                ...
                args=(dataset_obj, entity_obj),
                daemon=True
            )
            worker.start()
            return api_response(status=200, message='批量更新已开始，后台处理中')

        # 参数不全
        return api_response(status=400, message='缺少 dataset 或 entity 或 detail')

    def _run_batch_evaluation(self, dataset_obj, entity_obj):
        """
        异步批量评测——
        内部错误仅更新每条 entry 的状态，不再向前端直接返回。
        """
        entries = AiEvaluationDatasetEntry.objects.filter(dataset=dataset_obj)
    ...
    def _build_body_with_exec(self, original_body: dict, query: str) -> dict:
        """
        用 exec+repr 在字面量层面替换 '{{query}}'
        """
        body = copy.deepcopy(original_body)
        body_repr = repr(body).replace("'{{query}}'", repr(query))
        namespace = {}
