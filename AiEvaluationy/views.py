...
class AiEvaluationRecordServer(APIView):
    def post(self, request):
        ...
        # 2. 验证数据集
        try:
            dataset = AiEvaluationDataset.objects.get(pk=dataset)
        ...
        # 4. 验证条目一致性 & 获取 entity
        entries = AiEvaluationDatasetEntry.objects.filter(dataset=dataset)
        ...
        record = AiEvaluationRecord.objects.create(name=f"{entity_name}-{dataset_name}-{ts}",dataset=dataset_name,agent=",".join(agent_names),entity=entity_name,)

        serializer = AiEvaluationRecordSerializer(record)
        # 6. 启动后台线程
        worker = threading.Thread(target=self._run_evaluate_by_agents,args=(record.id, dataset, agent),daemon=True)
        worker.start()

        # 7. 返回整个 record 数据，而不仅是 id
        return api_response(status=200,message="评估已开始，可在历史记录中查看结果",data={"record": serializer.data})

    def _run_evaluate_by_agents(self, record, dataset_obj, agent_ids):
        """
        异步线程：对每个条目调用各智能体评分，并写入 AiEvaluationRecordEntry
        """
        try:
            record = AiEvaluationRecord.objects.get(pk=record)
            entries = AiEvaluationDatasetEntry.objects.filter(dataset=dataset_obj)
            for entry in entries:
                single = {
                    "question": entry.question,"ground_truth": entry.ground_truth or "","contexts": entry.contexts or "","answer": entry.answer or ""}
            ...
            logger.exception(f"异步评测出错 record_id={record.id}：{ex}")
    ...
    def post(self, request):
       ...
       开启分页查询
       {
        ...
        if record is not None:
            ...
            request.data['pageRule'].append({'field': 'record','rule': 'is','value': record,})

        print("DEBUG pageRule:", request.data.get('pageRule'))

        return pagination_query(cls=AiEvaluationRecordEntry,cls_serializer=AiEvaluationRecordEntrySerializer,request=request)
...
class ExcelImportView(APIView):
    ...
    def post(self, request):
        ...
        for rec in records:
            ...
            AiEvaluationDatasetEntry.objects.create(question=q.strip(), ground_truth=g, answer=answer,
                                                     contexts=contexts, update_time=parsed_dt, tag=tag_obj,
                                                     entity=entity_obj, dataset=dataset_obj)
            created_count += 1

        # 6. 返回结果，data 永远有值
        return api_response(status=200, message=f"导入完成：{created_count} 条新建，{skipped_count} 条跳过",
        ...
        if entity and detail:
            ...
            if resp.get('answer'):
                ...
                return api_response(
                    status=200,
                    message='单条数据评测完成',
                    data={'detail_id': detail, 'answer': resp['answer']}
                )
            else:
                entry.status = 'false'
